//! Cursor render pipeline — instanced rendering of remote user cursors.
//!
//! Renders all cursors in ONE draw call using GPU instancing.
//! Each cursor is a small pointer shape rendered via SDF in the fragment shader.
//!
//! Performance target: 1000 cursors < 100μs frame impact.
//!
...
use crate::vertex::{CameraUniform, CursorInstance, QuadVertex};

/// Maximum cursor instances per draw call (1000 users × 40B = 40 KB).
const MAX_CURSORS: usize = 1024;

/// Owns the wgpu pipeline, buffers, and bind groups for cursor rendering.
...
impl CursorPipeline {
    /// Create the cursor pipeline and allocate GPU buffers.
    pub fn new(device: &Device, surface_format: TextureFormat) -> Self {
        ...
        let pipeline = device.create_render_pipeline(&RenderPipelineDescriptor {
            ...
            vertex: VertexState {
                ...
                buffers: &[QuadVertex::layout(), CursorInstance::layout()],
            },
        ...
        let instance_buffer = device.create_buffer(&BufferDescriptor {
            ...
            size: (MAX_CURSORS * std::mem::size_of::<CursorInstance>()) as u64,
            usage: BufferUsages::VERTEX | BufferUsages::COPY_DST,
            mapped_at_creation: false,
        });
    ...
    /// Upload cursor instance data for this frame.
    pub fn upload_instances(&mut self, queue: &Queue, instances: &[CursorInstance]) -> u32 {
        let count = instances.len().min(MAX_CURSORS);
        if count == 0 {
    ...
    ///
    /// **One draw call** for all cursor instances.
    ...
    /// Number of cursor instances that will be drawn.

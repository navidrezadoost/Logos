...
│   └── package.json           # @logos-mcp/common package
...
│   └── package.json           # Includes @logos-mcp/common dependency
├── logos-plugin/             # Logos plugin with response capability
│   ├── src/
...
3. Implement the corresponding task handler class in the plugin (`logos-plugin/src/task-handlers/`).
...
4. Register the task handler in `logos-plugin/src/plugin.ts` in the `taskHandlers` list.
## Key Components
...
- **Type Safety**: Shared definitions via @logos-mcp/common package
### Core Classes

...
use logos_plugins::runtime::{PluginValue, Sandbox};
use std::sync::{Arc, RwLock};

fn bench_sandbox_create(c: &mut Criterion) {
    c.bench_function("sandbox_create", |b| {
        b.iter(|| {
            let s = Sandbox::new(black_box("bench-plugin"), logos_plugins::ResourceLimits::default());
            black_box(s);
        });
    });
}
...
fn bench_eval_literal(c: &mut Criterion) {
    let mut sandbox = Sandbox::new("bench", logos_plugins::ResourceLimits::default());
...
fn bench_host_get_layers(c: &mut Criterion) {
    ...
    for _ in 0..10 {
        doc.add_layer(Layer::Rect(RectLayer::new(0.0, 0.0, 100.0, 100.0)))
            .unwrap();
    }
...
fn bench_manager_load(c: &mut Criterion) {
    ...
    c.bench_function("manager_load_plugin", |b| {
        b.iter(|| {
